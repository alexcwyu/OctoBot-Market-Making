# Migration Guide: OctoBot-Market-Making
...
### 3. Source Layout
- **Layout**: `src/` layout
- **Package moved**: `octobot_market_making/` -> `src/octobot_market_making/`
- **Import unchanged**: `import octobot_market_making` still works

If upstream adds files to the old location, move them to `src/octobot_market_making/`.
...
known-first-party = ["octobot_market_making"]
```
...
### 6. Entry Points
- Console script `octobot_market_making` configured in `[project.scripts]`
- Was: `entry_points` in `setup.py`
...
3. **Check source layout**: If upstream adds new modules to the old path, move them to `src/octobot_market_making/`.
4. **Re-lock**: Run `uv lock --prerelease=allow` to update `uv.lock` with any new/changed dependencies.
5. **Verify**: Run `uv sync && uv run python -c "import octobot_market_making" && uv run pytest` (if tests exist).

## Quick Commands

```bash
uv sync                                              # Install all deps
uv run python -c "import octobot_market_making"        # Verify import
uv run pytest                                         # Run tests
uv run ruff check .                                   # Lint
uv run ruff format .                                  # Format
uv lock --prerelease=allow                            # Re-generate lockfile
```
